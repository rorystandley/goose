...
import { buildStatusBlocks } from './status.js';
...
export function registerCommandHandlers(app) {
  app.command('/goose', async ({ command, ack, client }) => {
    ...
    // Status command — live snapshot of agent state
    if (/^status$/i.test(task)) {
      log.info('Status requested', { user: command.user_id, channel: command.channel_id });
      try {
        const blocks = await buildStatusBlocks();
        await client.chat.postMessage({
          channel: command.channel_id,
          blocks,
          text: `${config.AGENT_NAME} status`,
        });
      } catch (err) {
        log.error('Failed to build status', { error: err.message });
        await client.chat.postMessage({
          channel: command.channel_id,
          text: `❌ Failed to build status: ${err.message}`,
        });
      }
      return;
    }
